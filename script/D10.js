...
const me = { name: "Andrea", surname: "Marchica", age: 29 };
...
const deleteOne = function (string, boolean) {
  ...
  } else if (boolean === false) string = string.slice(0, -1);
  console.log(string);
};

deleteOne("Ciao sono Andrea", false);
...
const howManyDays = function () {
  ...
  let passedMilliseconds = Date.parse();
...
const deleteProp = function (obj, property) {
  const changedObj = delete obj.property;
  return parseInt(changedObj);
};

deleteProp(object, "numero");
...
const searchByTitle = function (string) {
  if (string === movies["Title"]) return movies;
};
...
const tdPrinter = function () {
  const arrayTd = Array.from(allTd);
  arrayTd.forEach((td) => {
    console.log(td);
  });
};
...
for (i=0; i < allTr; i++) {
  allTr[i].setAttribute('class') = 'test'
}
allTr()
// [EXTRA] JS Avanzato
